...
## 5. 追加要件に関する確認事項（自由学習モード・定着率改善）

> **追加日:** 2026-02-09

### Q5-1: 自由学習の導線はどこに配置するか ✅

回答:
(b) デッキ一覧の各デッキに常時表示の「学習する」ボタンを追加。

### Q5-2: 自由学習の対象カードの範囲は ✅

回答:
(a) デッキ内の全カードが対象（SRS スケジュールに関係なく全て表示）。

### Q5-3: 自由学習の結果は SRS パラメータに反映するか ✅

回答:
(a) 反映しない（学習カード数カウントのみ。SRS は「今日の復習」でのみ更新される）。

### Q5-4: 「今日の復習」と「自由学習」のカウントは分けるか ✅

回答:
(c) ダッシュボードには合算値を表示するが、内部的には区別する。

### Q5-5: カウントの表示場所はどこか ✅

回答:
カードの定着度の部分は今日の復習のみ、学習サマリーのみ更新。

### Q5-6: 定着率の新しい計算方法 ✅

回答:
(c) 既存の intervalDays ベース定着度チャートはそのまま維持しつつ、今日の復習セッションの評価結果に基づく定着率を別途表示する。

### Q5-7: 定着率の表示場所は ✅

回答:
(b) 既存チャートはそのまま残し、別途「今日の復習 定着率」セクションを追加。

### Q5-8: デッキ別定着率の表示場所は ✅

回答:
(c) 両方（ダッシュボードのデッキ一覧 + デッキ詳細ページ）。
...
回答:
Google + GitHub の 2 プロバイダ。NextAuth.js (Auth.js) を使用。

### Q6-2: データベースの構成 ✅

回答:
ローカル開発は SQLite（従来通り）、本番は Turso（libSQL / SQLite 互換クラウド DB）を使用。

### Q6-3: マルチユーザーデータ分離の方針 ✅

回答:
認証導入と同時に対応する（Deck, Card, StudyRecord に userId を追加）。

### Q6-4: ユーザーメトリクスの方針 ✅

回答:
デプロイ前に再度検討する。認証導入タスクには含めない。

### Q6-5: コミュニティ機能の着手条件 ✅

回答:
ユーザー認証 + ユーザー別データ取得が完了してから着手する。
